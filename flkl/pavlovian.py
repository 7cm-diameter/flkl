...
from utex.shceduler import SessionMarker, TrialIterator, blockwise_shuffle, mix, repeat
...
async def conditional_discrimination(agent: Agent, ino: Flkl, expvars: dict):
    ...
    flick_each_trial = blockwise_shuffle(
        repeat(go_nogo_signals, number_of_trial // len(go_nogo_signals) + 1),
        len(go_nogo_signals),
    )
    trials = TrialIterator(flick_each_trial[0:number_of_trial], itis)

    try:
        while agent.working():
            for i, flick, iti in trials:
...
if __name__ == "__main__":
    ...
    for board in available_boards:
        ...
        if board.serial_number == com_input_config.get("serial-number"):
            ...
            [flkl.pin_mode(i, PinMode.OUTPUT) for i in range(2, 10)]
